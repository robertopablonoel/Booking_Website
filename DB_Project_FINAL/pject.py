...
app = Flask(__name__, template_folder='/Users/robertonoel/Desktop/DB_Project_FINAL/')
app.static_folder = '/Users/robertonoel/Desktop/DB_Project_FINAL/css'
...
def render_spend():
    email = session['email']
    cursor = conn.cursor();
    query1 = "SELECT * FROM monthly_spend where email = %s and month >= date_format(DATE_ADD(Now(), INTERVAL -6 Month),'%%Y-%%m')"
    query2 = "SELECT sum(spending) FROM monthly_spend where email = %s and month >= date_format(DATE_ADD(Now(), INTERVAL -12 Month),'%%Y-%%m')"
    cursor.execute(query1, (email))
    data = cursor.fetchall()
    cursor.execute(query2, (email))
...
def render_purchase():
    cursor = conn.cursor();
    test_query = "select max(intticketID) from ticket"
    cursor.execute(test_query)
    data = cursor.fetchall()
    session['ticket_no'] = str(int(data[0]['max(intticketID)']) + 1)
...
def purchase():
    ...
    cursor.execute(query, (ticketID, email, airline_name, flight_no, departure_date_time, sold_price, purchase_date, card_type,
                   card_no, name_on_card, exp_date, sec_code, intticketID))
    conn.commit()
    cursor.close()
